...
class CompoundV3SupplyFuse(Fuse):
  ...
  def create_fuse_enter_action(self, market_id: MarketId, amount: int) -> List[FuseActionDynamicStruct]:
    compound_v3_supply_fuse_enter_data = CompoundV3SupplyFuseEnterData(market_id.market_id, amount)
    return [FuseActionDynamicStruct(self.fuse_address, compound_v3_supply_fuse_enter_data.function_call())]

  def create_fuse_exit_action(self, market_id: MarketId, amount: int) -> List[FuseActionDynamicStruct]:
    compound_v3_supply_fuse_exit_data = CompoundV3SupplyFuseExitData(market_id.market_id, amount)
    return [FuseActionDynamicStruct(self.fuse_address, compound_v3_supply_fuse_exit_data.function_call())]
