...
class FluidInstadappSupplyFuse(Fuse):
  ...
  CLAIM = "claim"
  MAX_UINT256 = (
                    1 << 256) - 1  # Equivalent to BigInteger.valueOf(2).pow(Uint256.MAX_BIT_LENGTH).subtract(BigInteger.ONE)
  ...
  def __init__(
    ...
    self.fluid_instadapp_pool_token_address = self._require_non_null(fluid_instadapp_pool_token_address,
    ...
    self.fluid_instadapp_staking_contract_address = self._require_non_null(fluid_instadapp_staking_contract_address,
                                                                           "fluidInstadappStakingContractAddress is required")
    self.fluid_staking_fuse_address = self._require_non_null(fluid_instadapp_staking_fuse_address,
                                                             "fluidInstadappStakingFuseAddress is required")
    self.fluid_instadapp_claim_fuse_address = self._require_non_null(fluid_instadapp_claim_fuse_address,
  ...
  def supports(self, market_id: MarketId) -> bool:
    ...
    return market_id.protocol_id == self.PROTOCOL_ID and market_id.market_id == self.fluid_instadapp_pool_token_address
  ...
  def create_fuse_enter_action(self, market_id: MarketId, amount: int) -> List[FuseActionDynamicStruct]:
    ...
    fluid_staking_enter_data = FluidInstadappStakingSupplyFuseEnterData(self.MAX_UINT256,
    ...
    return [FuseActionDynamicStruct(self.erc4626_fuse_address, erc4626_supply_fuse_enter_data.function_call()),
            FuseActionDynamicStruct(self.fluid_staking_fuse_address, fluid_staking_enter_data.function_call())]

  def create_fuse_exit_action(self, market_id: MarketId, amount: int) -> List[FuseActionDynamicStruct]:
    fluid_instadapp_staking_exit_data = FluidInstadappStakingSupplyFuseExitData(amount,
                                                                                self.fluid_instadapp_staking_contract_address)
    erc4626_supply_fuse_exit_data = Erc4626SupplyFuseExitData(market_id.market_id, self.MAX_UINT256)

    return [FuseActionDynamicStruct(self.fluid_staking_fuse_address, fluid_instadapp_staking_exit_data.function_call()),
  ...
  def create_fuse_claim_action(self, market_id: MarketId) -> List[FuseActionDynamicStruct]:
    ...
    return [FuseActionDynamicStruct(self.fluid_instadapp_claim_fuse_address, claim_data)]
...
class FluidInstadappStakingSupplyFuseEnterData:
  ...
  def encode(self) -> bytes:
    return encode(['uint256', 'address'], [self.fluid_token_amount, self.staking_pool_address])
