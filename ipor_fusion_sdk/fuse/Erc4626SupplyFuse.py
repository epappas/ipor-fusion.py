...
class Erc4626SupplyFuse(Fuse):
  ...
  def supports(self, market_id: MarketId) -> bool:
    ...
    return market_id.protocol_id == self.protocol_id and market_id.market_id == self.erc4626_address
  ...
  def create_fuse_exit_action(self, market_id: MarketId, amount: int) -> List[FuseActionDynamicStruct]:
    ...
    return [FuseActionDynamicStruct(self.fuse_address, exit_data.function_selector())]

  def create_fuse_claim_action(self, market_id: MarketId) -> List[FuseActionDynamicStruct]:
    raise NotImplementedError("Fuse claim action is not supported for generic ERC4626")
